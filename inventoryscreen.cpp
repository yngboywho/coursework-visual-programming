...
InventoryScreen::InventoryScreen(QWidget *parent, QVBoxLayout *layout)
    : QObject(parent), parentWidget(parent), layout(layout)
{}

void InventoryScreen::loadInventory()
{
    QLayoutItem *item;
    ...
    QSqlQuery query("SELECT id, name, count_training, count_warehouse FROM Inventory");
    while (query.next())
    {
        ...
        addTrainingButton->setStyleSheet(
            ...
            "   border-color: #80d4ff;"
            "}");
        countTrainingLabel->setStyleSheet("font-size: 12px;  color: black; padding: 10px; background-color: #fff; border-radius: 5px; border: 1px solid #ced4da;");
        ...
        countWarehouseLabel->setStyleSheet("font-size: 12px;  color: black; padding: 10px; background-color: #fff; border-radius: 5px; border: 1px solid #ced4da;");
        ...
        inventoryWidgets.append(itemWidget);
    }
}

void InventoryScreen::addInventoryItem()
{
    ...
    QSqlQuery query;
    query.prepare("INSERT INTO Inventory (name, count_training, count_warehouse) VALUES (:name, 0, 0)");
...
void InventoryScreen::deleteInventoryItem(const QString &name)
{
    if (name.isEmpty())
    {
        return;
    ...
    checkQuery.prepare("SELECT COUNT(*) FROM Inventory WHERE name = :name");
...
void InventoryScreen::updateCount(int id, const QString &column, int change, QLabel *label)
{
    ...
    QSqlQuery query;
    query.prepare(QString("UPDATE Inventory SET %1 = :value WHERE id = :id").arg(column));
...
void InventoryScreen::editInventoryItem()
{
    ...
    QString originalName = QInputDialog::getText(parentWidget,
                                                 tr("Редактировать элемент инвентаря"),
                                                 tr("Введите название элемента для редактирования:"),
                                                 QLineEdit::Normal, "", &ok);
    ...
    QSqlQuery queryFind;
    queryFind.prepare("SELECT id, name, count_training, count_warehouse FROM Inventory WHERE name = ?");
    queryFind.addBindValue(originalName);

    if (!queryFind.exec() || !queryFind.next())
    {
        QMessageBox::warning(parentWidget, tr("Не найдено"),
                             tr("Элемент с таким названием не найден в базе данных."));
        return;
    }
    ...
    QString newName = QInputDialog::getText(parentWidget, tr("Редактировать элемент инвентаря"),
                                            tr("Введите новое название:"), QLineEdit::Normal, currentName, &ok);
    if (!ok)
        return;

    int newCountTraining = QInputDialog::getInt(parentWidget, tr("Редактировать элемент инвентаря"),
                                                tr("Введите количество для тренировки:"),
                                                currentCountTraining, 0, 1000000, 1, &ok);
    if (!ok)
        return;

    int newCountWarehouse = QInputDialog::getInt(parentWidget, tr("Редактировать элемент инвентаря"),
    ...
    queryUpdate.prepare("UPDATE Inventory SET name = ?, count_training = ?, count_warehouse = ? WHERE id = ?");
    ...
    {
        loadInventory();
        QMessageBox::information(parentWidget, tr("Успешно"), tr("Данные элемента инвентаря успешно обновлены!"));
    }
    else
    {
        QMessageBox::critical(parentWidget, tr("Ошибка"),
                              tr("Не удалось обновить данные элемента инвентаря: %1").arg(queryUpdate.lastError().text()));
    }
}
